...
| Impersonating **every** surface, not only records | Not a choice (2026-09-14). A CentralConfig session drives OData and `Session.asmx`; every `Workspace.asmx` method and the admin console answer **551** to it whatever role it holds. Forms, pick lists, quick actions and the catalog therefore stay on the service account. Measured, not assumed: real analysts call those methods from Ivanti's own mobile client, so it is the session type Ivanti refuses, not the role |
